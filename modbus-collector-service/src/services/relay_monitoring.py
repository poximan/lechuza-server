...
class RelayMonitoringService:
    ...
    def __init__(
        ...
        self.query_diagnostics = RelayQueryDiagnostics(self.reader)
    ...
    def read_relay_status(self, relay_id: int) -> None:
        ...
        if osciloperturbogramas_reles_dao.next_refresh(internal_id) > timebox.utc_now():
            return
        try:
            ...
            if (
                ...
                or not self.metadata.session_ready(relay_id)
            ):
            ...
            fallas_reles_dao.replace_if_newer(
                internal_id, record.fault_number,
                timebox.utc_iso_milliseconds(record.fault_datetime), record.timestamp_format,
                record.current_phase_a, record.current_phase_b, record.current_phase_c, record.earth_current,
            )
            references = self.reader.read_disturbance_references(relay_id)
            for reference in references:
                if not self.observer_store.get_reles_enabled() or self._stop_event.is_set():
                    return
                ...
                # Una descarga parcial exitosa se reutiliza al reintentar.
                if (
                    cached
                    and cached.get("signature") == signature
                    and cached.get("downloaded_at")
                    and timebox.parse(cached["downloaded_at"])
                    + osciloperturbogramas_reles_dao.REFRESH_INTERVAL
                    > timebox.utc_now()
                ):
                    continue
                payload = self.reader.read_disturbance(relay_id, configuration, reference)
                finish = decode_disturbance_finish(payload["metadata"]["finish_words"], date_format)
                payload["metadata"]["finish_timestamp"] = timebox.utc_iso_milliseconds(finish)
            ...
            osciloperturbogramas_reles_dao.failed(internal_id, exc)
            self.logger.log(
                f"No se completo la descarga del rele {relay_id}: {exc}",
                origin="OBS/RELE",
            )
