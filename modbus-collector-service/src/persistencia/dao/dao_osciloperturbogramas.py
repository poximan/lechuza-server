...
class OsciloperturbogramasRelesDAO:
    ...
    def complete(self, relay_id: int, records: list[int]) -> None:
        ...
        with closing(get_db_connection()) as conn, conn:
            if records:
                ...
                conn.execute(
                    "DELETE FROM osciloperturbogramas_reles "
                    f"WHERE id_rele = ? AND registro NOT IN ({placeholders})",
                    (relay_id, *records),
                )
            else:
                conn.execute(
                    "DELETE FROM osciloperturbogramas_reles WHERE id_rele = ?",
                    (relay_id,),
                )
            conn.execute(
                """
    ...
    def failed(self, relay_id: int, error: Exception) -> None:
        retry = timebox.utc_iso_milliseconds(timebox.utc_now() + self.RETRY_INTERVAL)
        with closing(get_db_connection()) as conn, conn:
            conn.execute(
                """
                INSERT INTO actualizacion_registros_reles
