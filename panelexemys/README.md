...
- No accede directamente a SQLite; toda la informacion proviene de APIs (`MODBUS_COLLECTOR_API_BASE`, `PVE_API_BASE`, `ROUTER_SERVICE_BASE_URL`).
...
Ver `config.py`: URLs base (`MODBUS_COLLECTOR_API_BASE`, `PVE_API_BASE`, `ROUTER_SERVICE_BASE_URL`), credenciales MQTT (`MQTT_BROKER_*`), destinatarios de alarmas, links del dashboard y hosts del chequeo de correo. Todas deben estar presentes en `.env`.
...
- `panelexemys` consume por HTTP a `modbus-collector-service`, `pve-service`, `router-telef-service`, `charito-service` y `mensagelo`.
