...
class MicomModbusException(MicomReadError):
    def __init__(self, message: str, exception_code: int):
...
class MicomRelayReader:
    ...
    def _read(self, relay_id: int, address: int, count: int) -> list[int]:
        ...
        result = self.driver.read_holding_registers_result(
            address,
            count,
            relay_id,
        )
        ...
        if registers is None:
            status = (
                "sin_registros"
                if result.exception_code == 0x0F
                else "excepcion_modbus"
                if result.exception_code is not None
                else "sin_respuesta"
            )
            received_count = None
            error = result.error or "lectura no disponible"
        ...
        if result.exception_code is not None:
            raise MicomModbusException(
                f"Rele {relay_id}: excepcion Modbus 0x{result.exception_code:02X} "
                f"en {hex(address)} ({count} palabras): {error}",
    ...
    def read_disturbance_references(
        ...
        except MicomModbusException as exc:
            if exc.exception_code == 0x0F:
                return []
            raise
    ...
    def _read_selected_samples(
        ...
        for page in range(self.DISTURBANCE_PAGE_FIRST, last_page + 1):
            ...
            offset = 0
            while offset < valid_word_count:
                request_words = min(
                    self.MODBUS_MAX_READ_WORDS,
                    valid_word_count - offset,
                )
                page_values.extend(
                    self._read(
                        relay_id,
                ...
                offset += request_words
            result.extend(signed_word(value) for value in page_values)
        return result
    ...
    def _parse_disturbance_index(
        ...
        if len(words) != self.DISTURBANCE_INDEX_WORDS:
            raise MicomReadError(
                "El indice de perturbacion requiere siete palabras"
            )
        origin = words[5]
