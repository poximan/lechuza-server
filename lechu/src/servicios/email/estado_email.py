...
def start_email_health_monitor(logger: Logosaurio, mqtt_manager) -> None:
    """
    Bucle de monitoreo cada 300 s. Persiste el estado de salud del correo.
    """
    publisher = MqttTopicPublisher(logger=logger, manager=mqtt_manager)
    health_dao = EmailHealthDao()
    last_payload: Optional[Dict[str, Any]] = None

    while True:
        try:
            estados = _build_status(logger)
            health_dao.save(estados)
            enriched_payload = {
                **estados,
                "ts": timebox.utc_iso(),
            }
            if estados != last_payload:
                published = publisher.publish_json(
            ...
            try:
                logger.log(f"persistencia email_health error: {e}", origin="EMAIL/CHK")
            except Exception:
                pass
        finally:
            time.sleep(300)
