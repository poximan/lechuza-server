...
from src.web.navigation import lechu_navigation
...
class ReactApi:
    ...
    def _register_routes(self) -> None:
        ...
        self.blueprint.add_url_rule(
            "/exemys", "exemys", self.exemys_api.get, methods=["GET"]
        )
        self.blueprint.add_url_rule(
            "/grd", "grd", self.exemys_api.get_grd_detail, methods=["GET"]
        )
        self.blueprint.add_url_rule(
    ...
    def _require_protected() -> Any | None:
        if lechu_navigation.current_mode() != lechu_navigation.protected_mode:
            return jsonify({"error": "protected_mode_required"}), 403
        return None

    def navigation(self) -> Any:
        mode = lechu_navigation.current_mode()
        return jsonify(lechu_navigation.contract(mode))
