...
class EmailService:
    ...
    def send_test(self) -> dict[str, Any]:
        recipient = config.EMAIL_TEST_RECIPIENTS
        subject = (
            f"{config.EMAIL_TEST_SUBJECT_PREFIX}"
            "Email de Prueba (Lechu - backend)"
        )
        body = (
            "Este es un email de prueba enviado desde Lechu - backend. "
